...
4. complex_solver,让模型在解决复杂问题时选择调用另一个专用模型（尽量是多模态的）（现在是另两个了，会调用两个模型，在三轮共六次调用九分钟内给出答案或彻底超时，然后把答案交给一个简化模型过滤输出，最终输出结果，我的AI模型太辣鸡了现在这个插件也不用了
...
8. heiboxyard，基于 2646617098/heibox-comment-bot 和 shrimp9527/heyboxbot 项目的小黑盒社区帖子爬取与晚报分析制作，附带一个可视化操作编辑晚报的web端
