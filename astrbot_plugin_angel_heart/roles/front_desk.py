    ...
    async def cache_message(self, chat_id: str, event: AstrMessageEvent):
        ...
        for component in message_chain:
            if isinstance(component, At):
                at_qq = component.qq
                at_name = getattr(component, 'name', None)
            
                display = at_name if at_name else (f"用户{at_qq}" if at_qq else "某人")
                mention = f"@{display}"
        ...
        # 将前缀拼接到文本最前面
        if at_prefix:
            text_content = at_prefix + text_content
            logger.debug(f"AngelHeart[{chat_id}]: 添加 At 前缀后文本: '{text_content}'")
        # ----- 新增结束 -----
